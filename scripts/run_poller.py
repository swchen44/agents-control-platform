...
def make_reload(loop, disp, ext, config_path: str = "config.yaml"):
    ...
    **舊設定原封續用**(fail-safe)。
    ...
    def _reload():
        ...
        disp.timeout_retry_max = int(s_cfg.get("timeout_retry_max") or 0)  # 可 reload
...
def main(argv: list[str] | None = None) -> int:
    ...
    # config 可覆寫(source.bot_account_id),否則啟動時 my_uid() 解析一次
    # (cloud=accountId、dc=name;主題 L)。
    bot_id = source_cfg.get("bot_account_id") or src.my_uid()
    ...
    disp.timeout_retry_max = int(source_cfg.get("timeout_retry_max") or 0)  # timeout 重跑
